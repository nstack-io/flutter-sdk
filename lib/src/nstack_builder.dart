...
class NstackBuilder implements Builder {
  ...
  void _writeHeader(StringBuffer output) {
    ...
    output
        .writeln("import 'package:nstack/partial/section_key_delegate.dart';");
    output.writeln("");
  ...
  void _writeSections(String response, StringBuffer output) {
    ...
    languageJson.forEach((section, keys) {
      ...
      keys.forEach((k, v) {
        ...
        output.writeln(
            "\tString get $k => get(SectionKey(\"$rawSection\", \"$k\", \"$v\"));");
      });
      output.writeln('''
}
''');
  ...
  void _writeNStackConfig(
      String projectId, String apiKey, StringBuffer output) {
    output.writeln('''
final _config = NStackConfig(projectId: "$projectId", apiKey: "$apiKey");
  ...
  void _writeLanguageList(List<LocalizeIndex> languages, StringBuffer output) {
    ...
    languages.forEach((localizeIndex) {
      ...
      output.writeln(
          "\tLanguage(id: ${language.id}, locale: \"${language.locale}\", direction: \"${language.direction}\", isDefault: ${language.isDefault}, isBestFit: ${language.isBestFit}),");
    });
...
final _bundledTranslations = {''');
    ...
    await Future.forEach<LocalizeIndex>(languages, (localizeIndex) async {
      ...
      final content = (await repository.fetchLocalizationForLanguage(localizeIndex));
